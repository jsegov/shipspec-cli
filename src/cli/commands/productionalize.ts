...
export const productionalizeCommand = new Command("productionalize")
  ...
  .option("--enable-scans", "Run SAST scanners if available", false)
  .option("--categories <list>", "Filter to specific categories (comma-separated)")
  .option("--no-stream", "Disable streaming progress output", false)
  .option("--checkpoint", "Enable checkpointing for state persistence", false)
